...
# ジャーナルとタスクをカテゴリーごとに分ける
  ...
  def average_color
    ...
    # 色コードをRGB値に変換
    rgb_values = colors.map { |color| color.delete("#").scan(/../).map(&:hex) }

    # RGB値の平均を計算
    averaged_rgb = rgb_values.transpose.map { |component| (component.sum / component.size).round }

    # 平均RGB値を16進数カラーコードに変換
    "#%02x%02x%02x" % averaged_rgb
  end
